  ...
  fullHeight() {
    setTimeout(() => {
      ...
      if (cards) {
        ...
        const content_cards =
          element?.getElementsByClassName('content-cover-card');
        ...
        for (let c in cards) {
          ...
          for (let i in noImage) {
            if (image_cards[c] && noImage[i].style) {
              noImage[i].style.height = image_cards[c].offsetHeight + 'px';
            }
          }
          let cardHeight = 0;
          ...
          if (cardHeight) {
            ...
            console.log(this.id, this.global.heightCoverSlider[this.id]);
    ...
    if (!this.isLoading && (this.is_paginated || this.page === 0) && this.url) {
      ...
      await fetch(this.url + (this.is_paginated ? `/${this.page}` : ''))
        ...
        .then((data) => {
          ...
          this.segmentList = this.segmentList.concat(
            this.is_paginated ? data.results : data
          );
          this.page++;
